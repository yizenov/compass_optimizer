  ...
  while (true) {
    ...
    if (line[0] != '\0' && line[0] != '\\') {
      ...
      if (current_line.back() == ';') {
        ...
        std::cout << "server start time ms: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << std::endl;
        ...
        if (thrift_with_retry(kSQL, context, query.c_str())) {
          ...
          std::cout << "server end time ms: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << std::endl;
          ...
          std::cout << "client time ms: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << std::endl;
        } else {
          (void)backchannel(TURN_OFF, nullptr);
        }
      } else {
