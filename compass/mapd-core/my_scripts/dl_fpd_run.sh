...
#screen -S dl_fpd_run -dm -L sh -c 'time ./dl_fpd_run.sh' (saved in screenlog.0, -Logfile fpd_run.log)

server_run="../build/bin/mapd_server --num-gpus 1 --start-gpu 0 -p 7071 --http-port 7070 --data /home/yizenov/.mapd/data/"
...
#input_folder=../../../JOB_QUERIES_DL
input_folder=../../../DL_Cardinality_Queries

#output_folder=../../../JOB_RESULTS_NOFPD
#output_folder=../../../JOB_RESULTS_WITHFPD_CPU_DL
output_folder=../../../DL_Cardinality_RESULTS
...
do
  ...
  java_process=$(ps -u yizenov | grep java | awk '{print $1}')
  kill -9 ${java_process} & K5=$!
  wait -n
  wait $K5

  server_process=$(ps -u yizenov | grep '$USER_mapd_server' | awk '{print $1}')
