...
ExecutionResult RelAlgExecutor::executeRelAlgQueryFPD(const RelAlgNode* ra,
  ...
  executor_->string_dictionary_generations_ = computeStringDictionaryGenerations(ra); //TODO: heave operation
  auto queue_time_ms_update = timer_stop(clock_begin_update);
  std::cout << "time taken for FPD RelAlg Setup: " << queue_time_ms_update << " ms" << std::endl;
  executor_->table_generations_ = computeTableGenerations(ra);
  ScopeGuard restore_metainfo_cache = [this] { executor_->clearMetaInfoCache(); };

  // passing corresponding sketch to executor
  std::vector<unsigned > cols_to_join_vector;
  auto node_value = std::stoi(std::to_string(table_id_phy.first) + "88" + std::to_string(table_id_phy.second));
  for (auto it_cols = getNodeJoinColumns()[node_value].begin(); it_cols != getNodeJoinColumns()[node_value].end(); ++it_cols) {
    auto column_id = std::get<1>(*it_cols);
    cols_to_join_vector.emplace_back(column_id);
  }
  executor_->setSketch(std::make_pair(cols_to_join_vector, getSketchInstances()[node_value]));
  executor_->setTableInfo(table_id_phy);
  executor_->setQueryStartTime(get_query_start_time());
...
ExecutionResult RelAlgExecutor::executeWorkUnit(const RelAlgExecutor::WorkUnit& work_unit,
  ...
  auto node_value = std::stoi(std::to_string(table_info.first) + "88" + std::to_string(table_info.second));
  ...
  if (!eo.just_explain && can_use_scan_limit(ra_exe_unit) && !isRowidLookup(work_unit)) {
    ...
    if (getSessionInfo().fpd_enabled() && eo.fpd_max_count >= 0) {
      ...
      table_sketch_complexity.emplace_back(std::make_pair(node_value, no_columns * filter_count_all));
    ...
    if (filter_count_all >= 0) { // it has been here from MapD
...
ssize_t RelAlgExecutor::getFilteredCountAll(const WorkUnit& work_unit,
  ...
  if (table_infos.size() == 1 && table_infos.front().info.getNumTuplesUpperBound() <= 50000) {
    return table_infos.front().info.getNumTuplesUpperBound();  // TODO: elaboration is needed
  }
  const auto count =
  ...
  CHECK(count_ptr);
  if (*count_ptr < 0) std::cout << "NEGATIVE SELECTIVITY" << std::endl;
...
ssize_t RelAlgExecutor::getFilteredCountAllAndUpdateSketch(const WorkUnit& work_unit,
  ...
  for (auto col : cols_to_update) {
    const auto cd = executor_->catalog_->getMetadataForColumn(table_id, col + 1);  // columnId start from 1 //TODO: elaboration is needed
  ...
  CHECK(count_ptr);
  if (*count_ptr < 0)
    std::cout << "NEGATIVE SELECTIVITY" << std::endl;
  std::cout << "number of survived tuples: " << *count_ptr;
  std::cout << " out of " << table_infos.front().info.getNumTuples() << std::endl;
  int64_t queue_time_ms = timer_stop(clock_begin);
  std::cout << "time taken in CountAllAndUpdateSketch: " << queue_time_ms << " ms" << std::endl;
  return std::max(*count_ptr, int64_t(1));
}
...
void adjustPlanForMapD(const std::unordered_map<size_t, std::set<size_t>>& graph,
  ...
  unsigned node_idx = 0, last_leaf_idx = -1;
  ...
  for (auto it = optimal_traversal.begin(); it != optimal_traversal.end(); ++it) {
    ...
    if (node_info->second.size() == 1) { // a leaf node
      auto conn_node = *(node_info->second.begin());
      auto is_adjacent = connected_nodes.find(conn_node);
...
double estimate_join_size(
    ...
    std::unordered_map<size_t, FAGMS_Sketch*>& fagms_sketches,
    std::unordered_map<size_t, std::vector<std::tuple<size_t, size_t, size_t, size_t, size_t>>>& join_cols_info,
    std::unordered_multimap<size_t, std::pair<size_t, size_t>>& table_index_infos) {
  ...
  for (auto it = sorted_order.begin(); it != sorted_order.end(); ++it) {
    if (*it > 300000)
      std::cout << "incorrect t_id_and_node_id: " << join_tables << std::endl;
  }

  double join_size_estimation = 0.0;
...
//    std::cout << std::endl << "estimation of : " << join_tables << ", value = " << join_size_estimation << std::endl;
  } else if (tree_size == 2) {
//    std::cout << std::endl << "not merging sketches of : " << join_tables << std::endl;

//    std::cout << "\tStage 0 (init), Node: " << traversal_nodes[0] << ", joining attributes:";
    auto join_nodes = join_cols_info[traversal_nodes[0]];
    for (unsigned int join_idx = 0; join_idx < join_nodes.size(); join_idx++) {
      auto other_node = std::get<2>(join_nodes[join_idx]);
      if (traversal_nodes[1] == other_node) {
        base_sketch->Add_Sketch_Values(join_idx);
//        std::cout << " " << std::get<1>(join_nodes[join_idx]);
        break;
      }
    }
//    std::cout << std::endl << "\t" << *base_sketch << std::endl;

//    std::cout << std::endl << "\tStage 0 (init), Node: " << traversal_nodes[1] << ", joining attributes:";
    FAGMS_Sketch* other_sketch = fagms_sketches[traversal_nodes[1]];;
    join_nodes = join_cols_info[traversal_nodes[1]];
    for (unsigned int join_idx = 0; join_idx < join_nodes.size(); join_idx++) {
      auto other_node = std::get<2>(join_nodes[join_idx]);
      if (traversal_nodes[0] == other_node) {
        other_sketch->Add_Sketch_Values(join_idx);
//        std::cout << " " << std::get<1>(join_nodes[join_idx]);
        break;
      }
    }
//    std::cout << std::endl << "\t" << *other_sketch << std::endl;

    join_size_estimation = base_sketch->Size_Of_Join(other_sketch);
//    std::cout << std::endl << "estimation of : " << join_tables << ", value = " << join_size_estimation << std::endl;
//
//    std::cout << "traversal_nodes: " << sorted_order.size() << ", estimation of:";
//    for (auto it : sorted_order) {
//      auto table_order_idx = table_index_infos.find(it);
//      std::cout << " " << table_order_idx->second.first << "_" << table_order_idx->second.second;
//    }
//    std::cout << ", value = " << join_size_estimation << std::endl;

    if (std::isnan(join_size_estimation)) { std::cout << "IS NAN value found" << std::endl; }
    bottom_up_table[join_tables] = join_size_estimation;

  } else if (tree_size > 2) {
//    std::cout << "merging sketches of : " << join_tables;
    FAGMS_Sketch* group_sketches[tree_size - 1];

    for (unsigned int i = 0; i < tree_size; i++) {
      FAGMS_Sketch* other_sketch = fagms_sketches[traversal_nodes[i]];
      other_sketch->Set_Sketch_With_Max();
      auto join_nodes = join_cols_info[traversal_nodes[i]];

//      std::cout << std::endl << "\tStage 0 (init), Node: " << traversal_nodes[i] << ", joining attributes:";

      // in case of several usage of the same column in join predicates
      for (unsigned int intern_idx = 0; intern_idx < traversal_nodes.size(); intern_idx++) {
        for (unsigned int join_idx = 0; join_idx < join_nodes.size(); join_idx++) {
          auto other_node = std::get<2>(join_nodes[join_idx]);
          if (traversal_nodes[intern_idx] == other_node) {
            other_sketch->Choose_Sketch_Min_Val(join_idx);
//            std::cout << " " << std::get<1>(join_nodes[join_idx]);
            break;
          }
        }
...
//      std::cout << std::endl << "\t" << *other_sketch;
//      std::cout << std::endl << "\tStage 1, Node: " << traversal_nodes[i];

      other_sketch->Merge_Zero();
//      std::cout << std::endl << "\t" << *other_sketch << std::endl;

      if (i > 0) { group_sketches[i - 1] = other_sketch; }
    }
...
void get_distinct_graph_traversals(
    const std::unordered_map<size_t, std::set<size_t>>& graph,
    const size_t node, /// not a pointer/reference
    std::vector<size_t>& traversal_nodes,  // used as a stack and to used to keep the order
    std::set<size_t>& visited_nodes,
    std::vector<std::vector<size_t>>& traversals,
    std::unordered_map<size_t, double> all_adjacent_nodes, /// not a pointer/reference
    std::vector<size_t>& optimal_traversal,
    double* min_join_size_estimation,
    std::map<std::string, double>& bottom_up_table,
    std::unordered_map<size_t, FAGMS_Sketch*>& fagms_sketches,
    FAGMS_Sketch* base_sketch, std::vector<unsigned>& base_sketch_columns,
    double temp_join_size_estimation, /// not a pointer/reference
    int64_t& estimate_time, unsigned& traverse_bound,
    std::unordered_map<size_t, std::vector<std::tuple<size_t, size_t, size_t, size_t, size_t>>>& join_cols_info,
    unsigned& NODE_TRAVERSE_BOUND,
    std::unordered_multimap<size_t, std::pair<size_t, size_t>>& table_index_infos) {

    // comment if needed
  if (traverse_bound >= NODE_TRAVERSE_BOUND) { return; }  // this is global threshold that can be adjusted
  ...
  if (traversal_nodes.size() == graph.size()) {
    join_size_estimation = estimate_join_size(traversal_nodes, bottom_up_table,
            base_sketch, base_sketch_columns, fagms_sketches, join_cols_info, table_index_infos);

//    std::cout << "traversal_nodes: " << traversal_nodes.size() << ", estimation of :";
//    for (auto it : traversal_nodes) {
//      auto table_order_idx = table_index_infos.find(it);
//      std::cout << " " << table_order_idx->second.first << "_" << table_order_idx->second.second;
//    }
//    std::cout << ", intermediate: " << temp_join_size_estimation << ", last join: " << join_size_estimation;
//    std::cout << ", final intermediate: " << temp_join_size_estimation;
//    std::cout << ", min: " << *min_join_size_estimation << std::endl;
  ...
  if (temp_join_size_estimation + join_size_estimation < 0)
    std::cout << "NEGATIVE value appeared because of the exceed" << std::endl;

  // comment if needed
  if (NODE_TRAVERSE_BOUND != 1 && temp_join_size_estimation + join_size_estimation > *min_join_size_estimation &&
  ...
  std::set<size_t> new_adjacent_nodes(node_info->second.begin(), node_info->second.end());
  for (auto it = all_adjacent_nodes.begin(); it != all_adjacent_nodes.end(); ++it) {
    if (visited_nodes.find(it->first) == visited_nodes.end())
      new_adjacent_nodes.emplace(it->first);
  }
  all_adjacent_nodes.clear();

  std::vector<std::pair<size_t, double>> adj_nodes;
  for (auto it = new_adjacent_nodes.begin(); it != new_adjacent_nodes.end(); ++it) {
    if (visited_nodes.find(*it) == visited_nodes.end()) {
      auto local_join_size_estimation = 0.0;
      traversal_nodes.emplace_back(*it);
      if (traversal_nodes.size() > 1 && traversal_nodes.size() <= graph.size()) {
        local_join_size_estimation = estimate_join_size(traversal_nodes, bottom_up_table,
                base_sketch, base_sketch_columns, fagms_sketches, join_cols_info, table_index_infos);
        traversal_nodes.pop_back();
        adj_nodes.emplace_back(*it, local_join_size_estimation);
        all_adjacent_nodes.emplace(*it, local_join_size_estimation);
      }
    }
  }
  new_adjacent_nodes.clear();
  // sort the nodes by their estimations and start with the smallest
  std::sort(adj_nodes.begin(), adj_nodes.end(), [](auto& lhs, auto& rhs) { return lhs.second < rhs.second; });

  for (auto it = adj_nodes.begin(); it != adj_nodes.end(); ++it) {
    if (visited_nodes.find(it->first) == visited_nodes.end()) {
      get_distinct_graph_traversals(graph, it->first, traversal_nodes, visited_nodes,
              traversals, all_adjacent_nodes, optimal_traversal, min_join_size_estimation,
              bottom_up_table, fagms_sketches, base_sketch, base_sketch_columns, temp_join_size_estimation,
              estimate_time, traverse_bound, join_cols_info, NODE_TRAVERSE_BOUND, table_index_infos);
    }
  }
  all_adjacent_nodes.clear();
  adj_nodes.clear();
...
void exclude_sparse_sketches(std::vector<std::pair<size_t, unsigned int>>& sketch_complexities,
    std::unordered_map<size_t, std::set<size_t>>& join_graph,
    std::unordered_map<size_t, FAGMS_Sketch*>& sketch_instances,
    std::unordered_map<size_t, std::vector<std::tuple<size_t, size_t, size_t, size_t, size_t>>>& join_connections,
    std::vector<std::pair<size_t, unsigned int>>& excluded_nodes_with_complexity,
    std::unordered_map<size_t, std::set<size_t>>& excluded_edges,
                             unsigned& SKETCH_SPARSITY_BOUND) {

    std::set<size_t> excluded_nodes;

    // iterate over tables
    for (auto node = sketch_complexities.begin(); node != sketch_complexities.end(); ++node) {

      if (excluded_nodes.find(node->first) != excluded_nodes.end()) continue;

      auto sketch = sketch_instances[node->first];
      auto joins = join_connections[node->first];

      // iterate over all the joins of a table
      std::vector<size_t> join_indices;
      size_t sketch_idx = -1;
      for (auto join_info = joins.begin(); join_info != joins.end(); ++join_info) {
        sketch_idx++;
        unsigned max_b;  // maximum number of non-zero bucket values across the rows
        sketch->Max_Non_Zero_Buckets(sketch_idx, max_b);

        if (max_b < SKETCH_SPARSITY_BOUND) {
          auto other_node = std::get<2>(*join_info);
          auto other_sketch = sketch_instances[other_node];
          auto other_column_id_idx = -1;
          auto other_joins = join_connections[other_node];
          for (auto other_join_info = other_joins.begin(); other_join_info != other_joins.end(); ++other_join_info) {
            other_column_id_idx++;
            if (node->first == std::get<2>(*other_join_info)) break;
          }

          //saving the indices
          if (excluded_edges.find(node->first) == excluded_edges.end()) {
            std::set<size_t> edges;
            excluded_edges[node->first] = edges;
          }
          excluded_edges[node->first].emplace(other_node);

          if (excluded_edges.find(other_node) == excluded_edges.end()) {
            std::set<size_t> edges;
            excluded_edges[other_node] = edges;
          }
          excluded_edges[other_node].emplace(node->first);

          join_connections[other_node].erase(join_connections[other_node].begin() + other_column_id_idx);
          join_indices.emplace_back(sketch_idx);

          join_graph[node->first].erase(other_node);
          join_graph[other_node].erase(node->first);

          if (other_sketch->get_keys_no() == excluded_edges[other_node].size()) {
            excluded_nodes.emplace(other_node);
            join_graph.erase(other_node);
            join_connections.erase(other_node);
          }
        }
      }

      for (auto join_idx = join_indices.begin(); join_idx != join_indices.end(); ++join_idx) {
        join_connections[node->first].erase(join_connections[node->first].begin() + *join_idx);
      }

      if (excluded_edges.find(node->first) != excluded_edges.end() && sketch->get_keys_no() == excluded_edges[node->first].size()) {
        excluded_nodes.emplace(node->first);
        join_graph.erase(node->first);
        join_connections.erase(node->first);
      }
    }

    for (auto ex_node = excluded_nodes.begin(); ex_node != excluded_nodes.end(); ++ex_node) {
      auto node_index = -1;
      for (auto node = sketch_complexities.begin(); node != sketch_complexities.end(); ++node) {
        node_index++;
        if (*ex_node == node->first) {
            excluded_nodes_with_complexity.emplace_back(*node);
            break;
        }
      }
      sketch_complexities.erase(sketch_complexities.begin() + node_index);
    }
    std::sort(excluded_nodes_with_complexity.begin(), excluded_nodes_with_complexity.end(), [](auto& left, auto& right) { return left.second < right.second; });
}

std::vector<size_t> get_node_input_permutation(
    const std::vector<InputTableInfo>& table_infos,
    std::unordered_multimap<size_t, std::pair<size_t, size_t>>& table_index_infos,
    std::vector<std::pair<size_t, unsigned int>>& sketch_complexities,
    std::unordered_map<size_t, std::set<size_t>>& join_graph,
    std::unordered_map<size_t, FAGMS_Sketch*>& sketch_instances,
    std::unordered_map<size_t, std::vector<std::tuple<size_t, size_t, size_t, size_t, size_t>>>& join_connections,
    unsigned& NODE_TRAVERSE_BOUND, unsigned& SKETCH_SPARSITY_BOUND,
    std::set<size_t>& materialized_statuses) {
    auto clock_begin = timer_start();
    std::cout << "+++++++++++++++++++++++++++++++++++++++++++" << std::endl;
    std::cout << "enum start time ms: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << std::endl;

    std::vector<size_t> input_permutation(table_infos.size());
    std::iota(input_permutation.begin(), input_permutation.end(), 0);

    if (!join_connections.empty()) {

      ///////////////////////////////PRINTING FOR DL-CARDINALITY///////////////////////////////////
      bool dl_experiments = false;
      if (dl_experiments == true) {
          std::cout << "\nSTARTTT" << std::endl << std::endl;
//          std::ofstream data_for_dl;
          // /home/username/.mapd/data/ + ../../compass-sketch/MapD/mapd-core/QueryEngine/Sketch/
          // std::string PATH_TO_CATALOG_SK = get_catalog().get_basePath() + PATH_TO_SKETCHES + "dl_temp_file.txt";
//          std::string PATH_TO_CATALOG_SK = "../QueryEngine/Sketch/dl_temp_file.txt";
//          data_for_dl.open(PATH_TO_CATALOG_SK); // std::ios_base::app

          for (auto node = sketch_instances.begin(); node != sketch_instances.end(); ++node) {
              for (auto node_in = join_connections.begin(); node_in != join_connections.end(); ++node_in) {
                  if (node->first == node_in->first) {
                      for (auto join_info = node_in->second.begin(); join_info != node_in->second.end(); ++join_info) {
                          auto table_id = std::get<0>(*join_info);
                          auto column_id = std::get<1>(*join_info);
                          auto columnsNo = node->second->get_keys_no();
                          auto rowsNo = node->second->get_rows_no();
                          auto bucketsNo = node->second->get_buckets_no();

                          auto table_order_idx = table_index_infos.find(node_in->first);
                          unsigned node_tuples = table_infos[table_order_idx->second.second].info.getNumTuples();

                          std::cout << "table_id: " << table_id << " ++++ column_id: " << column_id << " ++++ tuples: " << node_tuples <<
                              " ++++ columnsNo: " << columnsNo << " ++++ rowsNo: " << rowsNo << " ++++ bucketsNo: " << bucketsNo << std::endl;
//                          std::string dl_sketch_info = std::to_string(table_id) + " " + std::to_string(column_id) + " " +
//                              std::to_string(node_tuples) + " " + std::to_string(columnsNo) + " " + std::to_string(rowsNo) + " " + std::to_string(bucketsNo) + "\n";
//                          data_for_dl << dl_sketch_info;
//
//                          auto column_index = 0; // column_index++;
//                          Xi_CW2B **xi_bucket_ = node->second->get_seed1_vals();
//                          Xi_EH3 **xi_pm1 = node->second->get_seed2_vals();
//                          double** separate_sketch_elem = node->second->get_separate_sketch_elements();
//
//                          for (unsigned rw = 0; rw < rowsNo; rw++) {
//                            data_for_dl << xi_bucket_[column_index][rw].seeds[0] << " " << xi_bucket_[column_index][rw].seeds[1] << " ";
//                          }
//                          data_for_dl << "\n";
//                          for (unsigned rw = 0; rw < rowsNo; rw++) {
//                            data_for_dl << xi_pm1[column_index][rw].seeds[0] << " " << xi_pm1[column_index][rw].seeds[1] << " ";
//                          }
//                          data_for_dl << "\n";
//
//                          for (unsigned rw = 0; rw < rowsNo * bucketsNo; rw++) {
//                            data_for_dl << separate_sketch_elem[column_index][rw] << " ";
//                          }
//                          data_for_dl << "\n\n";
                      }
                  }
          ...
          std::cout << std::endl;
          std::cout << "ENDDDDD" << std::endl << std::endl;
//          data_for_dl.close();
          return input_permutation;
      }
      ///////////////////////////////PRINTING FOR DL-CARDINALITY///////////////////////////////////

      std::unordered_map<size_t, std::set<size_t>> init_graph(join_graph.begin(), join_graph.end());

      // in order to start from small nodes, we create this data structure, size = number of tuples * number of join columns
      std::sort(sketch_complexities.begin(), sketch_complexities.end(), [](auto& left, auto& right) { return left.second < right.second; });
//      std::cout << std::endl << "INIT SKETCH COMPLEXITIES::" << std::endl;
//      for (auto node = sketch_complexities.begin(); node != sketch_complexities.end(); ++node) { std::cout << " " << node->first; }
//      std::cout << std::endl;

      std::unordered_map<size_t, std::vector<std::tuple<size_t, size_t, size_t, size_t, size_t>>> join_connections__;
      for (auto node = join_connections.begin(); node != join_connections.end(); ++node) { join_connections__[node->first] = node->second; }

      std::vector<std::pair<size_t, unsigned int>> excluded_nodes;
      std::unordered_map<size_t, std::set<size_t>> excluded_edges;
      // comment if needed
      exclude_sparse_sketches(sketch_complexities, join_graph, sketch_instances, join_connections__,
              excluded_nodes, excluded_edges, SKETCH_SPARSITY_BOUND);

//      //////////////////////////////////////START PRINT//////////////////////////////////////////////////////////////
//      std::cout << std::endl << "EXCLUDED NODES::" << std::endl;
//      for (auto node = excluded_nodes.begin(); node != excluded_nodes.end(); ++node) { std::cout << " " << node->first; }
//      std::cout << std::endl;
//
//      std::cout << std::endl << "EXCLUDED EDGES::" << std::endl;
//      for (auto node = excluded_edges.begin(); node != excluded_edges.end(); ++node) {
//        std::cout << "\t" << node->first << " ::";
//        for (auto edge = node->second.begin(); edge != node->second.end(); ++edge) { std::cout << " " << *edge; }
//        std::cout << std::endl;
//      }
//
//      std::cout << "\nINIT Join Graph" << std::endl;
//      for (auto node = join_graph.begin(); node != join_graph.end(); ++node) {
//        std::cout << "\t" << node->first << " ::";
...
//      for (auto node = join_connections.begin(); node != join_connections.end(); ++node) {
...
//      std::cout << std::endl;
//
//      std::cout << "CHANGED Join Connections" << std::endl;
//      for (auto node = join_connections__.begin(); node != join_connections__.end(); ++node) {
//        std::cout << "\tlhs_table_node: " << node->first << std::endl;
//        for (auto join_info = node->second.begin(); join_info != node->second.end(); ++join_info) {
//          auto table_id = std::get<0>(*join_info);
//          auto column_id = std::get<1>(*join_info), join_table_node = std::get<2>(*join_info);
...
//      //////////////////////////////////////END PRINT////////////////////////////////////////////////////////////////
      std::map<std::string, double> bottom_up_table;
//      std::vector<unsigned int> bound_limits = {1, 1, 10, 100}; //TODO:
      std::vector<unsigned int> bound_limits = {1}; //TODO:
//      bound_limits.emplace_back(NODE_TRAVERSE_BOUND);

      ///////// to get a node with the minimum two-way join estimate for a baseline
      auto base_clock_begin = timer_start();
      double min_two_way = std::numeric_limits<double>::max();
      std::pair<size_t, unsigned int> temp_node;
      std::vector<size_t> base_final_plan;
      bool consider_base_plan = true; //TODO: comment if needed
...
//      std::cout << std::endl;
      ...
      std::vector<std::pair<size_t, unsigned int>> largest_complexity(sketch_complexities.begin(), sketch_complexities.end());
      std::sort(largest_complexity.begin(), largest_complexity.end(), [](auto& left, auto& right) { return left.second > right.second; });

//      std::cout << "largest to smallest: ";
//      for (auto kk = largest_complexity.begin(); kk != largest_complexity.end(); ++kk) {
//          std::cout << kk->first << " " << kk->second << ", ";
//      }
//      std::cout << std::endl;
      ...
      for (auto kk = largest_complexity.begin(); kk != largest_complexity.end(); ++kk) {
//      for (auto kk = sketch_complexities.begin(); kk != sketch_complexities.end(); ++kk) {
//          std::cout << "start: " << kk->first << std::endl;
        std::vector<size_t> temp_nodes = {kk->first};
        FAGMS_Sketch *base_sketch = sketch_instances[temp_nodes[0]];
        std::vector<unsigned> base_sketch_columns;
        for (auto it_cols = join_connections[kk->first].begin(); it_cols != join_connections[kk->first].end(); ++it_cols) {
          auto column_id = std::get<1>(*it_cols);
          base_sketch_columns.emplace_back(column_id);
        }
        for (auto it_cols = join_connections[kk->first].begin(); it_cols != join_connections[kk->first].end(); ++it_cols) {
          temp_nodes.emplace_back(std::get<2>(*it_cols));
          double loc_join_size_estimation = estimate_join_size(temp_nodes, bottom_up_table, base_sketch,
                  base_sketch_columns, sketch_instances, join_connections, table_index_infos);
          if (loc_join_size_estimation <= min_two_way) {
            min_two_way = loc_join_size_estimation;
            auto left_table_order_idx = table_index_infos.find(temp_nodes[0]);
            unsigned left = table_infos[left_table_order_idx->second.second].info.getNumTuples();
            auto right_table_order_idx = table_index_infos.find(temp_nodes[1]);
            unsigned right = table_infos[right_table_order_idx->second.second].info.getNumTuples();
            if (left > right) {
//                std::cout << "chose: " << temp_nodes[0] << " than " << temp_nodes[1] << std::endl;
              temp_node = *kk;
            } /// else it chooses the largest of two tables
          }
          temp_nodes.pop_back();
        }
        temp_node = *kk;
      ...
      //tmp_sketch_complexities.emplace_back(temp_node);
      auto base_time_ms = timer_stop(base_clock_begin);
      std::cout << std::endl << "time taken for prepare base enumeration: " << base_time_ms << " ms" << std::endl;
      ////////////// end ///////////////////////////

      for (auto b_limit = bound_limits.begin(); b_limit != bound_limits.end(); ++b_limit) {
        bottom_up_table.clear();
        auto enum_clock_begin = timer_start();

        std::vector<size_t> optimal_traversal;
        double min_join_size_estimation = std::numeric_limits<double>::max();
        std::vector<std::vector<size_t>> traversals;  // generating all distinct traversals of the graph
        std::vector<size_t> traversal_nodes;
        std::set<size_t> visited_nodes;
        unsigned max_reach_node_traversal = 0;

        // starting with less complicated sketches
        for (auto kk = sketch_complexities.begin(); kk != sketch_complexities.end(); ++kk) {

          size_t base_node;
          if (consider_base_plan) {
            base_node = temp_node.first;
          } else {
            base_node = kk->first;
          }

          double temp_join_size_estimation = 0.0;
          FAGMS_Sketch *base_sketch = sketch_instances[base_node];  // getting the base case sketch for the current node
          std::vector<unsigned> base_sketch_columns;
          for (auto it_cols = join_connections[base_node].begin(); it_cols != join_connections[base_node].end(); ++it_cols) {
            auto column_id = std::get<1>(*it_cols);
            base_sketch_columns.emplace_back(column_id);
          }

          int64_t total_join_est_time = 0;
          unsigned traversal_bound = 0;

          std::unordered_map<size_t, double> all_adjacent_nodes;
          get_distinct_graph_traversals(join_graph,
                    base_node,
                    traversal_nodes,
                    visited_nodes,
                    traversals,
                    ...
                    *b_limit,
                    table_index_infos);
          if (traversal_bound > max_reach_node_traversal) {
              max_reach_node_traversal = traversal_bound;
          }
//          std::cout << "base node:: " << base_node << " " << traversal_bound << ", min est: "
//              << min_join_size_estimation << std::endl << std::endl;
          all_adjacent_nodes.clear();
          base_sketch_columns.clear();

           // comment if needed
          if (consider_base_plan) {
            for (auto it : optimal_traversal) { base_final_plan.emplace_back(it); }
            break;
          }
        }

        if (consider_base_plan) {
          consider_base_plan = false;
          std::cout << std::endl << "BOUND LIMIT: " << *b_limit
            << " BASE, number of potential optimal plans without cartesian products: >" << traversals.size() << std::endl;
        } else {
          std::cout << std::endl << "BOUND LIMIT: " << *b_limit
            << ", number of potential optimal plans without cartesian products: >" << traversals.size() << std::endl;
        }
        if (traversals.size() < 1) { std::cout << "NO TRAVERSAL WAS FOUND" << std::endl; }

        if (max_reach_node_traversal >= *b_limit) { std::cout << "BFS has reached the traverse bound, max: "
            << max_reach_node_traversal << std::endl; }
        else { std::cout << "BFS hasn't reached the traverse bound, max: " << max_reach_node_traversal << std::endl; }
        ...
        auto enum_time_ms = timer_stop(enum_clock_begin);
        std::cout << "time taken for enumeration: " << enum_time_ms << " ms" << std::endl;

        std::vector<size_t> temp_nodes = {optimal_traversal[0]};
        FAGMS_Sketch *base_sketch = sketch_instances[temp_nodes[0]];
        std::vector<unsigned> base_sketch_columns;
        for (auto it_cols = join_connections[temp_nodes[0]].begin(); it_cols != join_connections[temp_nodes[0]].end(); ++it_cols) {
          auto column_id = std::get<1>(*it_cols);
          base_sketch_columns.emplace_back(column_id);
        }

        double total_temp = 0;
        for (unsigned i = 1; i < optimal_traversal.size(); i++) {
          temp_nodes.push_back(optimal_traversal[i]);
          double loc_join_size_estimation = estimate_join_size(temp_nodes, bottom_up_table, base_sketch,
                  base_sketch_columns, sketch_instances, join_connections, table_index_infos);
          total_temp += loc_join_size_estimation;
          auto table_order_idx = table_index_infos.find(temp_nodes[0]);
          std::cout << "intermediate join size estimation of: " << table_order_idx->second.first;
          for (unsigned j = 1; j < temp_nodes.size(); j++) {
            auto table_order_idx = table_index_infos.find(temp_nodes[j]);
            std::cout << " <> " << table_order_idx->second.first;
          }
          std::cout << " = " << loc_join_size_estimation << std::endl;
        }
        temp_nodes.clear();
        base_sketch_columns.clear();
        std::cout << "total estimation sum of the intermediate result: " << total_temp << std::endl;

        std::vector<size_t> final_plan(optimal_traversal.begin(), optimal_traversal.end());
        std::set<size_t> already_added_excluded_nodes;

        if (excluded_nodes.size() > 0) std::cout << "reconstructing final plan..." << std::endl;

        // append all small relations that connected to a given node right away
        auto ex_node = excluded_nodes.begin();
        while (excluded_nodes.size() != already_added_excluded_nodes.size()) {
          if (ex_node == excluded_nodes.end()) ex_node = excluded_nodes.begin();
          if (already_added_excluded_nodes.find(ex_node->first) != already_added_excluded_nodes.end()) {
            ex_node++;
            continue;
          }
          ...
          for (auto node = final_plan.begin(); node != final_plan.end(); ++node) {
            input_idx++;
            auto joins = join_connections[*node];
            for (auto join_info = joins.begin(); join_info != joins.end(); ++join_info) {
              auto join_table_node = std::get<2>(*join_info);
              if (join_table_node == ex_node->first) {
                found_node = true;
                final_plan.insert(final_plan.begin() + input_idx, ex_node->first);
                already_added_excluded_nodes.emplace(ex_node->first);
                break;
              }
            }
            if (found_node) break;
        ...
        already_added_excluded_nodes.clear();

        std::cout << "final full plan:";
        ...
        final_plan.clear();
        for (auto it : base_final_plan) { final_plan.emplace_back(it); }

        adjustPlanForMapD(init_graph, final_plan);
        std::cout << "tweaked for MapD full plan:";
        input_permutation.clear();
        for (auto it = final_plan.begin(); it != final_plan.end(); ++it) {
          auto table_order_idx = table_index_infos.find(*it);
          std::cout << " " << table_order_idx->second.first << "_" << table_order_idx->second.second;
          input_permutation.emplace_back(table_order_idx->second.second);
        }
        std::cout << std::endl;
      ...
      std::sort(input_permutation.begin(), input_permutation.end(), [&table_infos](const size_t lhs_index, const size_t rhs_index) {
    ...
    std::cout << std::endl << "time taken for finding an optimal plan: " << queue_time_ms << " ms" << std::endl;
    std::cout << "enum end time ms: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << std::endl;
    std::cout << "+++++++++++++++++++++++++++++++++++++++++++" << std::endl;

//    int c = 256; // TODO: TO STOP THE EXECUTION PART
//    if (c > std::numeric_limits< char> ::max())
//        throw std::invalid_argument("The argument too large.");
    return input_permutation;
}
  ...
  if (left_deep_join) {
    if (g_from_table_reordering &&
      ...
      const auto input_permutation = get_node_input_permutation(
          query_infos, getTableIndexInfo(), getSketchComplexity(), getJoinGraph(), getSketchInstances(),
          getNodeJoinColumns(), getNodeTraverseBound(), getSketchSparsityBound(), getMaterializedStatuses());
