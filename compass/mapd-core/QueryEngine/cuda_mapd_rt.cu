...
extern "C" __device__ void agg_count_and_update_sketch_gpu2(int64_t* counter,
  ...
  sketch_data[block_set + threadIdx.x] += 1; // TODO: counters are accurate, how?
...
extern "C" __device__ void agg_count_and_update_sketch_gpu1(int64_t* counter,
  ...
  //printf("blockDim %d, block %d, thread %d\n", blockDim.x, blockIdx.x, threadIdx.x);

//  printf("num_cols_to_update %d, num_buckets %d, num_rows %d\n", num_cols_to_update, num_buckets, num_rows);
//  printf("keys:");
//  for (uint16_t i = 0; i < num_cols_to_update; i++) {
//    printf(" %d", keys[i]);
//  }
//  printf("\n");
//
...
//  printf("\n");
//
//  printf("gpu xi_bucket:");
//  for (uint16_t i = 0; i < num_rows; i++) {
//    for (uint16_t j = 0; j < num_cols_to_update; j++) {
//      printf(" %u - %u", xi_bucket[2 * (num_cols_to_update * i + j)], xi_bucket[2 * (num_cols_to_update * i + j) + 1]);
//    }
//  }
//  printf("\n");
//
...
//  for (uint16_t i = 0; i < num_rows; i++) {
//    for (uint16_t j = 0; j < num_cols_to_update; j++) {
//      printf(" %u - %u", xi_pm1[2 * (num_cols_to_update * i + j)], xi_pm1[2 * (num_cols_to_update * i + j) + 1]);
//    }
//  }
//  printf("\n");
}
