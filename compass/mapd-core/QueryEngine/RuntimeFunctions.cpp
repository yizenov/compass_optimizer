...
extern "C" ALWAYS_INLINE int64_t* agg_count_and_update_sketch(int64_t* counter,
                                                              ...
                                                              const uint16_t num_rows) {
//  if (counter[0] == 0)
//    printf("other way of getting thread id = %d\n", pthread_self());

  // updating sketch
  uint64_t sketch_size = num_buckets * num_rows;
  uint32_t row_first_modulo = pthread_self() % num_rows; //TODO: negative id values? # std::this_thread::get_id()
  for (uint16_t i = 0; i < num_rows; i++) {
    ...
    for (uint16_t j = 0; j < num_cols_to_update; j++) {
      ...
      sketch_data[bucket_idx] += EH3(xi_pm1[first_seed_idx], xi_pm1[second_seed_idx], keys[j]) * 1.0;
    }
  }
