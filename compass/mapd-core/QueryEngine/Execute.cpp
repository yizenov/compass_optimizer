...
int32_t Executor::executePlanWithoutGroupBy(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  if (device_type == ExecutorDeviceType::CPU) {
    ...
    std::cout << "CPU version is being processed" << std::endl;
    auto query_start_time = timer_start();
    ...
    for (const auto target_expr : target_exprs) {
      ...
      if (agg_info.agg_kind == kFAGMS_SKETCH) {
        ...
        auto sketch_cpu_init_begin = timer_start();
        uint64_t data_alloc_size = getSketch().second->get_rows_no() * getSketch().second->get_buckets_no() * getSketch().second->get_keys_no();
        std::cout << "array size: ~" << data_alloc_size * 4 / 1000 << " KB, sep.sketch nbr: " << getSketch().second->get_keys_no() << std::endl;
        std::vector<int64_t> vec(data_alloc_size, 0);
        query_exe_context->init_agg_vals_ = vec;
        auto sketch_cpu_init_stop = timer_stop(sketch_cpu_init_begin);
        std::cout << "time taken for initial sketch values setup in cpu: " << sketch_cpu_init_stop << " ms" << std::endl;
        break;
      }
    }
    ...
    auto sketch_cpu_init_stop = timer_stop(sketch_cpu_init_begin);
    std::cout << "time taken for actual sketch update in cpu: " << sketch_cpu_init_stop << " ms" << std::endl;

    auto copy_built_sketch_begin = timer_start();
    if (iskFAGMS_SKETCH) {
      ...
      for (unsigned int i = 0; i < column_number; i++) {
        ...
        for (unsigned int j = 0; j < sketch_size; j++) {
          column_sketch[j] = query_exe_context->init_agg_vals_[col_sketch_size + j];
        }
    ...
    auto copy_built_sketch_stop = timer_stop(copy_built_sketch_begin);
    std::cout << "time taken for copying freshly built sketch from cpu: " << copy_built_sketch_stop << " ms" << std::endl;

    output_memory_scope.reset(new OutVecOwner(out_vec));
    auto query_end_time = timer_stop(query_start_time);
    std::cout << "time taken for entire query processing: " << query_end_time << " ms" << std::endl;
  } else {
    try {
      OOM_TRACE_PUSH();

      std::cout << "GPU version is being processed" << std::endl;
      auto query_start_time = timer_start();
      bool iskFAGMS_SKETCH = false;
      std::vector<int64_t> initial_agg_vals = query_exe_context->init_agg_vals_;
      for (const auto target_expr : target_exprs) {
        ...
        if (agg_info.agg_kind == kFAGMS_SKETCH) {
          ...
          uint64_t data_alloc_size = gridSize() * (blockSize() + getSketch().second->get_rows_no() * getSketch().second->get_buckets_no() * getSketch().second->get_keys_no());
          std::cout << "array size: ~" << data_alloc_size * 4 / 1000000 << " MB, sep.sketch nbr: " << getSketch().second->get_keys_no() << std::endl;
          std::vector<int64_t> vec(data_alloc_size, 0);
          query_exe_context->init_agg_vals_ = vec;
          auto sketch_gpu_init_stop = timer_stop(sketch_gpu_init_begin);
          std::cout << "time taken for initial sketch values setup in gpu: " << sketch_gpu_init_stop << " ms" << std::endl;
          break;
        }
      }
      ...
      auto sketch_gpu_init_stop = timer_stop(sketch_gpu_init_begin);
      std::cout << "time taken for actual sketch update in gpu: " << sketch_gpu_init_stop << " ms" << std::endl;
      ...
      auto copy_built_sketch_stop = timer_stop(copy_built_sketch_begin);
      std::cout << "time taken for copying freshly built sketch from gpu: " << copy_built_sketch_stop << " ms" << std::endl;
      output_memory_scope.reset(new OutVecOwner(out_vec));
  ...
  if (size_t(1) == entry_count) {
    ...
    for (const auto target_expr : target_exprs) {
      ...
      if (is_distinct_target(agg_info)) {
        ...
        if (device_type == ExecutorDeviceType::GPU && agg_info.agg_kind == kFAGMS_SKETCH) {
...
//        } else if (device_type == ExecutorDeviceType::CPU && agg_info.agg_kind == kFAGMS_SKETCH) {
        } else {
          std::tie(val1, error_code) = reduceResults(agg_info.agg_kind,
                                                   agg_info.sql_type,
                                                   query_exe_context->init_agg_vals_[out_vec_idx],
...
int32_t Executor::executePlanWithGroupBy(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  if (device_type == ExecutorDeviceType::CPU) {
    ...
    try {
      std::cout << "materializing the push-down result for current table..." << std::endl;
